    ...
    void Update()
    {
        ...
        if (player)
        {
            ...
            if (panel.activeSelf)
            {
                ...
                finesseText.text = player.finesse.value.ToString();
                finesseButton.interactable = player.finesse.PointsSpendable() > 0;
                finesseButton.onClick.SetListener(() => {
                    player.finesse.CmdIncrease();
                });
            }
        }
