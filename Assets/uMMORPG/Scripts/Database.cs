...
public partial class Database : MonoBehaviour
{
    ...
    public string CharacterDNA(string character, int teem)
    {
        characters row = connection.FindWithQuery<characters>("SELECT * FROM characters WHERE name=? AND deleted=0", character);
        return row.dna;
    }

    public List<string> CharacterDNA(string account)
    {
        List<string> result = new List<string>();
        foreach (characters character in connection.Query<characters>("SELECT * FROM characters WHERE account=? AND deleted=0", account))
        {
            result.Add(character.dna);
        }
        return result;
    }
    ...
    public GameObject CharacterLoad(string characterName, List<Player> prefabs, bool isPreview)
    {
        ...
        if (row != null)
        {
            ...
            if (prefab != null)
            {
                ...
                player.finesse.value                          = row.finesse;
