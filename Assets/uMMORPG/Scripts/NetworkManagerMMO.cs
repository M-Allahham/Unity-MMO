...
public partial class NetworkManagerMMO : NetworkManager
{
    ...
    void LoadPreview(GameObject prefab, Transform location, int selectionIndex, CharactersAvailableMsg.CharacterPreview character)
    {
        ...
        //player.dna = character.dna;
        player.GetComponent<DynamicCharacterAvatar>().LoadFromRecipeString(character.dna);
    ...
    void OnClientCharactersAvailable(NetworkConnection conn, CharactersAvailableMsg message)
    {
        ...
        for (int i = 0; i < charactersAvailableMsg.characters.Length; ++i)
        {
            ...
            datadna = character.dna;
            // Store DNA maybe on network manager? and reference from player
            //GameObject.Find(character.name).GetComponent<Player>().dna = character.dna;
            ...
            if (prefab != null)
            {
                ...
                while (prefab.gameObject.GetComponent<DynamicCharacterAvatar>() == null)
                {
                    prefab.gameObject.GetComponent<DynamicCharacterAvatar>().LoadFromRecipeString(character.dna);
                }
            }
            else
    ...
    Player CreateCharacter(GameObject classPrefab, string characterName, string account, string dna, bool gameMaster)
    {
                    ...
                    if (Database.singleton.CharactersForAccount(account).Count < characterLimit)
                    {
                        ...
                        if (0 <= message.classIndex && message.classIndex < playerClasses.Count)
                        {
                                ...
                                conn == NetworkServer.localConnection)
                            {
                                // create new character based on the prefab.
                                print(message.dna);
                                Player player = CreateCharacter(playerClasses[message.classIndex].gameObject, message.name, account, message.dna, message.gameMaster);
    ...
    void OnServerCharacterSelect(NetworkConnection conn, CharacterSelectMsg message)
    {
        ...
        if (lobby.ContainsKey(conn))
        {
            ...
            if (0 <= message.index && message.index < characters.Count)
            {
                ...
                print("Adding dna: " + Database.singleton.CharacterDNA(characters[message.index], 1));
                go.GetComponent<DynamicCharacterAvatar>().LoadFromRecipeString(Database.singleton.CharacterDNA(characters[message.index], 1));
