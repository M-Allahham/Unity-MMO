...
#pragma warning restore CS0109 // member does not hide accessible member
    ...
    public LogInt baseDamage = new LogInt{baseValue=10};
    public LogInt baseDefense = new LogInt{ baseValue=10};
    ...
    [Server]
    public virtual void DealDamageAt(Entity victim, double amount, float stunChance=0, float stunTime=0)
    {
        ...
        if (!victimCombat.invincible)
        {
            ...
            else
            {
                ...
                damageDealt = (int)(amount * (100.0 / (100.0 + (double)victimCombat.defense)) + 1);
