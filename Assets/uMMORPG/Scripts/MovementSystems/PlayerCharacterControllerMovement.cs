...
public class PlayerCharacterControllerMovement : Movement
{
    ...
    public PlayerTabTargeting targeting;
    ...
    void UpdateCamera()
    {
        ...
        if (mouseRotationLocksCursor)
        {
            ...
            // otherwise unlock in any case
                ...
                if (player.target?.GetComponent<Monster>() != null && player.target?.health.current > 0)
                {
                    // This is where we adjust the camera to target focus!
                    //cameraPivotTransform = player.transform;
                    Vector3 dir = player.target.transform.position - player.transform.position;
                    dir.Normalize();
                    dir.y = 0;

                    Quaternion targetRotation = Quaternion.LookRotation(dir);
                    transform.rotation = targetRotation;
                    dir = player.target.transform.position;
                    dir.Normalize();
