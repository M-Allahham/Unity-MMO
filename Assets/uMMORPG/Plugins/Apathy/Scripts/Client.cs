...
namespace Apathy
{
    public class Client : Common
    {
        ...
        // calling this function will Disconnect the client and fully handle the
        // disconnect before returning (= OnDisconnect(), cleanup, etc.)
        public void Disconnect()
        {
            ...
            if (Connecting || Connected)
            {
                ...
                // DO NOT clean up yet.
                // we want Update to realize that the socket was closed, then
                // call OnDisconnected, then clean up all the state.
                // if we clean up now, then Connected is false and Update never
                // realizes that we disconnected.
                //CleanUp();

                // Disconnect should be synchronous. we closed the socket, but
                // Connected is still true and OnDisconnected isn't called until
                // the next Update(), so let's call Update here once to handle
                // everything.
                // (otherwise all tests would need Disconnect(); Update();)
                Update();
            }
        }
